...
async def _call_llm_with_semaphore(
    ...
    """Call the judge with concurrency control via semaphore."""
    semaphore = asyncio.Semaphore(max_concurrent)

    logger.debug("Judge messages: %s", messages)
    async with semaphore:
        return await _call_llm(messages, judge_call, max_retries)
...
async def _call_llm(
    ...
    """Invoke the injected judge callable with retry/backoff.

    ``judge_call`` is a ``JudgeCallable`` (sync or async) returning the
    assistant's reply as a string. The result is wrapped in OpenAI-style
    ``{"choices": [{"message": {"content": ...}}]}`` for downstream code
    that already speaks that shape.
    """
    for attempt in range(max_retries):
        try:
            result = judge_call(messages)
            if asyncio.iscoroutine(result):
                result = await result
            if not isinstance(result, str):
                raise TypeError(
                    f"judge_call must return str, got {type(result).__name__}"
                )
            return {"choices": [{"message": {"content": result}}]}
        except Exception as e:
            traceback.print_exc()
            logger.warning(
                "Judge call failed (attempt %d/%d): %s", attempt + 1, max_retries, e
            )
